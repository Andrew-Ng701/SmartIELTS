package com.andrew.smartielts.listening.constants;

public final class ListeningConstants {

    private ListeningConstants() {
    }

    public static final String TIMER_MODE_TEST_LEVEL = "test_level";

    public static final Integer DEFAULT_PREP_SECONDS = 0;
    public static final Integer DEFAULT_TOTAL_SECONDS = 3600;
    public static final Integer DEFAULT_AUTO_SUBMIT = 1;
    public static final Integer DEFAULT_ALLOW_PAUSE = 0;
    public static final Integer DEFAULT_ALLOW_AUDIO_SEEK = 0;

    public static final Integer NOT_DELETED = 0;
    public static final Integer DELETED = 1;
}
